...
{
    /// <summary>Priority and time tick for unconnected request timeout.</summary>
    ...
    /// <summary>Raw Forward Open service data bytes (from priority/tick to end of connection path).</summary>
    public ReadOnlyMemory<byte> RawServiceData { get; init; }
    ...
    public static ForwardOpenRequest Parse(ReadOnlySpan<byte> data, bool isLarge = false)
    {
        ...
        return new ForwardOpenRequest
        {
            ...
            RawServiceData = data.Slice(0, offset + pathBytes).ToArray(),
        };
    }
}
